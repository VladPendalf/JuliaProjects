...
=#    
    ...
    while !isborder(robot,side)
        ...
        if ismarker(robot)==true 
            num_markers += 1 # - это тоже самое, что и if ismarker(robot) num_markers += 1 end
...
=#
    ...
    while !isborder(robot,start_side[2]) || is_posible!(robot, start_side[1], invers(start_side[2]), num_steps, side_steps) # <=> Робот - не в стартовом углу
        for i in 1:2
